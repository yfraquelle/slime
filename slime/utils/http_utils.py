...
async def _post(client, url, payload, headers=None, max_retries=60):
    retry_count = 0
...
def _init_ray_distributed_post(args):
    ...
    class _HttpPosterActor:
        ...
        async def do_post(self, url, payload, headers=None, max_retries=60):
            return await _post(self._client, url, payload, headers, max_retries)
...
async def post(url, payload, headers=None, max_retries=60):
    ...
    if _distributed_post_enabled and _post_actors:
        try:
            ...
            if actor is not None:
                ...
                obj_ref = actor.do_post.remote(url, payload, headers, max_retries)
                return await asyncio.to_thread(ray.get, obj_ref)
        except Exception as e:
            logger.info(f"[http_utils] Distributed POST failed, falling back to local: {e} (url={url})")
            # fall through to local

    return await _post(_http_client, url, payload, headers, max_retries)
